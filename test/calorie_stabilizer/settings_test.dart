...
void main() {
  group('StabilizerSettings', () {
    test('default settings have valid values', () {
      const settings = StabilizerSettings();
      expect(settings.isValid, isTrue);
      expect(settings.targetDailyCalories, equals(2000));
      expect(settings.windowHours, equals(24));
      expect(settings.minDailyCalories, equals(1200));
      expect(settings.maxDailyCalories, equals(3000));
    });

    test('soft preset has relaxed values', () {
      final settings = StabilizerSettings.soft();
      expect(settings.compensationRate, equals(0.3));
      expect(settings.maxCompensationPerDay, equals(150));
      expect(settings.minDailyCalories, equals(1500));
    });

    test('strict preset has aggressive values', () {
      final settings = StabilizerSettings.strict();
      expect(settings.compensationRate, equals(0.7));
      expect(settings.maxCompensationPerDay, equals(400));
    });

    test('bulking preset has higher targets', () {
      final settings = StabilizerSettings.bulking();
      expect(settings.targetDailyCalories, equals(2800));
      expect(settings.minDailyCalories, equals(2200));
    });

    test('cutting preset has lower targets', () {
      final settings = StabilizerSettings.cutting();
      expect(settings.targetDailyCalories, equals(1600));
      expect(settings.minDailyCalories, equals(1200));
    });

    test('validation catches invalid settings', () {
      final invalidSettings = const StabilizerSettings(
        targetDailyCalories: 1000,
        minDailyCalories: 1500, // больше чем target
      );
      expect(invalidSettings.isValid, isFalse);
      expect(invalidSettings.validate(), isNotEmpty);
    });

    test('copyWith creates modified copy', () {
      const original = StabilizerSettings();
      final modified = original.copyWith(targetDailyCalories: 2500);

      expect(original.targetDailyCalories, equals(2000));
      expect(modified.targetDailyCalories, equals(2500));
      expect(modified.windowHours, equals(original.windowHours));
    });
  // });
  //
  // group('CalorieEntry', () {
  //   test('creates entry with required fields', () {
  //     final timestamp = DateTime(2025, 6, 1, 9, 0);
  //     final entry = CalorieEntry(timestamp: timestamp, calories: 500);
  //
  //     expect(entry.timestamp, equals(timestamp));
  //     expect(entry.calories, equals(500));
  ...
  //     );
  //
  //     expect(entry.note, equals('Завтрак'));
  //   });
  //
  //   test('copyWith creates modified copy', () {
  ...
  //       calories: 500,
  //     );
  //     final modified = original.copyWith(calories: 600);
  //
  //     expect(original.calories, equals(500));
  //     expect(modified.calories, equals(600));
